...
#	norm_hh,prev_hh = [],[]
	...
	nn = open('test_signal.csv','r')
	nnline = csv.reader(nn)

#	pp = open('error_hh.csv','r')
#	ppline = csv.reader(pp)

        tt = open('test_signal.csv','r')
        ttline = csv.reader(tt)

	jj = open('test_signal.csv', 'r')
	jjline = csv.reader(jj)

	kk = open('test_signal.csv', 'r')
	kkline = csv.reader(kk)

	rr = open('test_signal.csv', 'r')
	rrline = csv.reader(rr)

	for line in nnline:
		if 'Bjet1_pT' in line: continue
		norm_hh.append(float(line[-21])) # -2:topness, -3:higgsness
...
#                norm_tt.append(np.log10(float(line[-2])))
        ...
        for line in ttline:
                if 'Bjet1_pT' in line: continue
                norm_tt.append(float(line[-17]))
               
	for line in jjline:
		if 'Bjet1_pT' in line: continue
		norm_jj.append(float(line[-3]))
...
#	h1 = TH1D("h1", "1D Distribution on Delta R", 140, -7, 7)
#	h2 = TH1D("h2", "1D Distribution on Delta R", 140, -7, 7)
...
#	h1 = TH1D("h1", "1D Distributions on W Mass", 30, 0, 300)
#	h2 = TH1D("h2", "1D Distributions on W Mass", 30, 0, 300)
#	h3 = TH1D("h3", "1D Distributions on H Mass", 30, 0, 300)	

	h4 = TH1D("h4", "1D Distribution on W Mass", 30, 0, 300)
	h5 = TH1D("h5", "1D Distribution on W Mass", 30, 0, 300)
	
	h6 = TH1D("h6", "1D Distribution on W Mass", 30, 0, 300)

	h_sig = TH1D("h_sig", "for measure std", 1, 0, 1)
...
#	lst_hh.AddEntry(h1, "dR_WW", "l")
#	lst_hh.AddEntry(h2, "dR_jj", "l")

	lst_hh.AddEntry(h3, "H to lvqq (Neutrino opt used)","l")        
