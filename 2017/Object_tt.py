...
#	flist = glob.glob('/u/user/lgg0819/mc/GluGluToHHTo2B2WToLNu2J_node_SM_TuneCUETP8M1_PSWeights_13TeV-madgraph-pythia8/NANOAODSIM/PUMoriond17_Nano02Apr2020_102X_mcRun2_asymptotic_v8-v1/*/*')
	flist = glob.glob('/u/user/lgg0819/mc/RunIIFall17NanoAODv7/GluGluToHHTo2B2WToLNu2J_node_SM_TuneCP5_PSWeights_13TeV-madgraph-pythia8/NANOAODSIM/PU2017_12Apr2018_Nano02Apr2020_102X_mc2017_realistic_v8-v1/*/*')
...
#                        if (hlt_mu_1 == 1 or hlt_mu_2 == 1 or hlt_mu_3 == 1 or hlt_mu_4 == 1 or hlt_mu_5 == 1 or hlt_mu_6 == 1): hlt_mu = True
			if (hlt_mu_5 == 1 or hlt_mu_7 == 1): hlt_mu = True
...
#				if (flag_el == True): break
			...
			Sub_pt, Sub_eta, Sub_phi, Sub_M = [], [], [], []
			for j in range(len(sub_pt)):
				if (sub_pt[j] > 30 and abs(sub_eta[j]) < 2.4 and sub_btag[j] > 0.6321):
					Sub_pt.append(sub_pt[j])
					Sub_eta.append(sub_eta[j])
					Sub_phi.append(sub_phi[j])
					Sub_M.append(sub_mass[j])
			...
			Fat_pt, Fat_eta, Fat_phi, Fat_M = [], [], [], []

			for j in range(len(fat_pt)):
				if (fat_pt[j] > 200 and abs(fat_eta[j]) < 2.4 and fat_btag[j] > 0.6321 and fat_sdm[j] > 30 and fat_sdm[j] < 210 and fat_tau2[j]/fat_tau1[j] < 0.75):
					for k in range(len(Sub_pt)):
						if (  math.sqrt((fat_eta[j] - Sub_eta[k])**2 + (fat_phi[j] - Sub_phi[k])**2) > 0.8): continue
						dR_Fat.append(  math.sqrt((fat_eta[j] - sub_eta[k])**2 + (fat_phi[j] - sub_phi[k])**2)  )
					if (len(dR_Fat) < 2): continue
					if (llep_pt > 0 and math.sqrt((fat_eta[j] - llep_eta)**2 + (fat_phi[j] - llep_phi)**2) <= 0.8): continue
					if (el_pt > 0 and math.sqrt((fat_eta[j] - el_eta)**2 + (fat_phi[j] - el_phi)**2) <= 0.8): continue

					Fat_pt.append(fat_pt[j])
					Fat_eta.append(fat_eta[j])
					Fat_phi.append(fat_phi[j])
					Fat_M.append(fat_mass[j])

			# =================================================================
			Event_case = 0

			if (len(Fat_pt) > 0): Event_case = 1
#			if (len(Fat_pt) == 0): Event_case.append(0)

			# Event case = 0: rseolved, Event case = 1: boosted
			...
			for j in range(len(thin_pt)):
				...
				if (thin_pt[j] > 25 and abs(thin_eta[j]) < 2.4 and thin_jetid[j] >= 6):
			...
			for j in range(len(cand_res_pt1)):
				if (cand_res_pt1[j] > 25 and abs(cand_res_eta1[j]) < 2.4 and cand_res_jetid1[j] >= 6):
					j_check += 1
			...
			if (b_check == 1):
				BJet_1.SetPtEtaPhiM(sel_bpt[0], sel_beta[0], sel_bphi[0], sel_bmass[0])
				BJet_2.SetPtEtaPhiM(0, 0, 0, 0)

			# ---------- Event preselection criteria (HH)----------

			if (met_pt == 0): continue
			if (llep_pt == 0 and el_pt == 0): continue
			if (llep_pt > 0 and el_pt > 0): continue
			if (lep1_p4.Pt() == 0): continue
			if (b_check < 1): continue
			if (had_check < 1): continue
			if (had_check < 2 and b_check < 2): continue
			# ----------------------------------------
			...
			# Save CSV file	
			data.append([Fat_pt, Fat_eta, Fat_phi, Fat_M, # 19 ~ 22
					Event_case, # 18 [0 = Resolved, 1 = Boosted]
					sel_bpt, sel_beta, sel_bphi, sel_bmass, # 14 ~ 17
					sel_pt,sel_eta,sel_phi,sel_mass, # 10 ~ 13
					lep1_p4.Pt(),lep1_p4.Eta(),lep1_p4.Phi(),lep1_p4.M(), # 6 ~ 9
					met_p4.Pt(), met_p4.Eta(), met_p4.Phi(), met_p4.M(), # 2 ~ 5
					passed_event]) # 1

        df = pd.DataFrame(data, columns= ['fatjet_pt', 'fatjet_eta', 'fatjet_phi', 'fatjet_mass',
					'event_case',
					'bjet_cand_pt', 'bjet_cand_eta', 'bjet_cand_phi', 'bjet_cand_mass',
					'jet_cand_pt','jet_cand_eta','jet_cand_phi','jet_cand_mass',
